...
function App() {
    ...
    const { data: todos = [], isPending, isFetching, isError, error } = useQuery({
        queryKey: ['todos'],
        queryFn: async () => {
    ...
    const addTodoMutation = useMutation({
        mutationFn: async (text: string) => {
            const response = await axios.post<TodoDTO>(URL_API, {text: text, complited: false, createAt: Date.now()});
            return response.data;
        },
        onSuccess: () => {
